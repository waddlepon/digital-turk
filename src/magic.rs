...
const ROOK_MAGICS: [u64; 64] = [1157990257514250242, 2882875507851132992, 1161398080045057, 4899934021695569920,
2594095375631523840, 9241387612189753344, 7097690677972336640, 4647714958296093184, 9016021654700032,
4701969254651068416, 13837362910931730432, 5189272756673331201, 1125968694608904, 148618925230329856,
4621256446842109952, 79657506474835968, 1376805519364, 180148384225592324, 176059302608900,
2305844108995919872, 138043457536, 36064532220805120, 9223373137473732608, 8806880773632,
2253041596235780, 4582774129295360, 1126466851028992, 2305984984461082624, 1153000673756315652, 13835093445825267714,
4612249801671770112, 3485857047268886016, 2305984201976971264, 9227950403281256456, 4507997675978752,
4582910495817728, 2019864570318946305, 4415293493248, 88513774126080, 595056242712444928,
71064866652168, 9081970877530116, 1172114828951552, 286431520030720, 2306142097855741984,
2306002455579852800, 2882310358603857924, 4755802308166631424, 2342153556693894144, 885420464866112,
144150922491068448, 18288575778848, 1152941433322226688, 2306969212453652480, 1441154081141850368,
6598143542400, 721418207646212, 18296595300356, 4656735245365690372, 8899241574404,
4612812056042012676, 18159121858644, 87969528611848, 9223373138279023620,]; 

const BISHOP_MAGICS: [u64; 64] = [9523160548515053568, 9376784978861375492, 589415583842304, 1171221798688129538,
36108237304498690, 185219906429517824, 564668948021248, 1152921651039780940, 90142370049427456,
4683964073145204736, 10017136023911809024, 4612134638515716228, 1161428950077568, 9295452737921613824,
4614500777053126656, 2314851310264123920, 4612811978632593408, 2323857450807214080, 4503702719184896,
288230376843776002, 1152921522005345409, 11710484931338830082, 347905304622792714, 1125917359931392,
14074024033320960, 5193794412542525444, 108719712988340232, 73324231449919785, 577094071034589312,
937100572673458176, 4802675380392017, 11529777996303110160, 27586747014578176, 288555866135724088,
1300559529548840960, 1153062929374117940, 25473622864830468, 99171550778900480, 4611969692444131328,
82190968077422601, 2254028943761476, 11529928234049208384, 9229848546900967424, 1126045935738880,
13907186020211754496, 2318230116039131136, 4836902369587494928, 9225633768782495744, 9223936297880518656,
9226822484750237696, 10415126226150440, 54043205238276100, 4611826893388255881, 2306019034186910216,
92358981363695616, 580967792216899716, 2882393410386526232, 18067454375067648, 306246494418632720,
4617456337059315744, 360297969142071304, 9223486661277646848, 149920196626432, 90076425054331904,];
...
impl MagicBoards{
    fn blockermask_rook(square: usize) -> u64 {
        ...
        if rank != 0 {
            for i in 0..(rank - 1) { blockermask |= 1 << (file + i * 8) };
        }
        for i in (file + 1)..7 { blockermask |= 1 << (i + rank * 8) };
        if file != 0 {
            for i in 0..(file - 1) { blockermask |= 1 << (i + rank * 8) };
        }
    ...
    fn find_magic(&mut self, square: usize, rook: bool) {
        ...
        loop {
            ...
            if GEN_MAGICS {
                ...
                if ((magic.wrapping_mul(self.rook_magic_table[square].mask)) >> 56).count_ones() < 6 { continue; }
            }
            else {
                if rook {
    ...
    pub fn magic_move_rook(&self, square: usize, occupancy: u64) -> u64 {
        let index = ((occupancy & self.rook_magic_table[square].mask) * self.rook_magic_table[square].magic) >> self.rook_magic_table[square].shift;

        self.moveboards_rook[square][index as usize]        
    }

    pub fn magic_move_bishop(&self, square: usize, occupancy: u64) -> u64 {
        let index = ((occupancy & self.bishop_magic_table[square].mask) * self.bishop_magic_table[square].magic) >> self.bishop_magic_table[square].shift;
